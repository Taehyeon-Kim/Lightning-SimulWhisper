...
from .mlx_whisper import decoding, load_models, tokenizer
from .mlx_whisper.audio import N_FRAMES, N_SAMPLES, TOKENS_PER_SECOND, log_mel_spectrogram, pad_or_trim
...
def _log_encoding_time(encoder_type: str, duration_ms: float):
    ...
    with open(ENCODING_CSV, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([time.time(), encoder_type, duration_ms])
...
def _log_decoding_time(infer_id: int, step: int, duration_ms: float, total_tokens: int):
    ...
    with open(DECODING_CSV, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([time.time(), infer_id, step, duration_ms, total_tokens])
...
def _log_infer_time(infer_id: int, total_duration_ms: float, num_decode_steps: int):
    ...
    with open(INFER_CSV, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([time.time(), infer_id, total_duration_ms, num_decode_steps])
...
class MLXInference:
    ...
    def logits(self, tokens, audio_features):
        logits, self.kv_cache, cross_attentions = self.model.decoder(tokens, audio_features, kv_cache=self.kv_cache)
...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        if not self.use_coreml_encoder:
            logger.info(
                f"Using MLX encoder - Model encoder conv1 weight dtype: {self.model.encoder.conv1.weight.dtype}"
            )
        else:
            logger.info("Using CoreML encoder - MLX encoder will be bypassed")

        self.decode_options = decoding.DecodingOptions(language=cfg.language, without_timestamps=True, task=cfg.task)
        ...
        self.CIFLinear, self.always_fire, self.never_fire = load_cif(cfg, n_audio_state=self.model.dims.n_audio_state)
        self.dec_attns = []
        ...
        if cfg.decoder_type == "greedy":
            ...
            self.token_decoder = decoding.BeamSearchDecoder(
                inference=self.inference, eot=self.tokenizer.eot, beam_size=cfg.beam_size
            )
    ...
    def init_context(self):
        kw = {"tokenizer": self.tokenizer, "prefix_token_ids": [self.tokenizer.sot_prev]}
    ...
    def init_tokens(self):
        ...
        self.initial_tokens = mx.array(self.tokenizer.sot_sequence_including_notimestamps, dtype=mx.int64)[None, :]
