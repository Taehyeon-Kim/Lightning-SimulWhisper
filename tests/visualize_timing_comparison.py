...
def create_encoding_comparison(data):
    ...
    for impl in combined_df["implementation"].unique():
        impl_data = combined_df[combined_df["implementation"] == impl]["duration_ms"]
        fig.add_trace(
            go.Box(y=impl_data, name=f"{impl.upper()}", marker_color=COLORS.get(impl, "#888888"), boxpoints="outliers"),
            row=1,
            col=2,
        )
    ...
    # Update layout
    fig.update_layout(title="Encoding Performance Comparison", height=800, showlegend=True)
...
def create_decoding_comparison(data):
    ...
    # Update layout
    fig.update_layout(title="Decoding Performance Comparison", height=800, showlegend=True)
...
def create_inference_comparison(data):
    ...
    # Update layout
    fig.update_layout(title="Inference Performance Comparison", height=800, showlegend=True)
...
def create_comprehensive_summary(data):
    ...
    fig = make_subplots(
        rows=1, cols=3, subplot_titles=["Encoding Performance", "Decoding Performance", "Inference Performance"]
    )
    ...
    fig.update_layout(title="Comprehensive Performance Summary", height=500, showlegend=False)
...
def create_scaling_analysis(data):
    ...
    fig = make_subplots(
        rows=1, cols=2, subplot_titles=["Decoding Time vs Sequence Length", "Inference Time vs Complexity"]
    )
    ...
    fig.update_layout(title="Performance Scaling Analysis", height=500, showlegend=True)
