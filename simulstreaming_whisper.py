...
def simulwhisper_args(parser):
    ...
    group.add_argument("--audio_max_len", type=float, default=30.0, help="Max length of the audio buffer, in seconds.")
    group.add_argument(
        "--audio_min_len",
        type=float,
        default=0.0,
    ...
    group.add_argument(
        "--init_prompt", type=str, default=None, help="Init prompt for the model. It should be in the target language."
    )
    group.add_argument(
        "--static_init_prompt",
        type=str,
    ...
    group.add_argument(
        "--max_context_tokens", type=int, default=None, help="Max context tokens for the model. Default is 0."
    )
...
def simul_asr_factory(args):
    ...
    asr_kwargs["language"] = args.lan
    asr_kwargs["segment_length"] = args.min_chunk_size
    asr_kwargs["decoder_type"] = decoder
    ...
    logger.info(f"Arguments: {asr_kwargs}")
    asr = SimulWhisperASR(**asr_kwargs)
...
class SimulWhisperASR(ASRBase):
    ...
    def use_vad(self):
        logger.warning("VAD not implemented")
...
class SimulWhisperOnline(OnlineProcessorInterface):
    ...
    def timestamped_text(self, tokens, generation):
        ...
        tokens = tokens.copy()
        ret = []
        for sw, st in zip(split_words, split_tokens, strict=False):
            b = None
            for stt in st:
    ...
    def process_iter(self):
        ...
        t_start = time.time()
        logger.debug("[PERF] SimulWhisperOnline.process_iter() started")
        ...
        e = self.end if self.is_last else ts_words[-1][1] + self.audio_bufer_offset
...
if __name__ == "__main__":
    ...
    if os.environ.get("MLX_MEMORY_LOG"):
        try:
            ...
            memory_logger.start()
            logger.info(f"MLX memory logging enabled: {log_file}")
        except ImportError:
            logger.warning("mlx_memory_monitor not found, memory logging disabled")
